...
function sendSearchInsumo() {
    ...
    datos_search['search_insumo'] = document.getElementById('search_insumo').value;
...
function mandarFormularioInsumo(operation, operation2, formulario, add_button, button_cancel) {
    if (verifyForm()) {
...
function confirmarEliminarInsumo() {
    if (confirm('Esta seguro de querer eliminar este insumo?')) {
        token_operation = document.forms['form_operation'].elements['csrfmiddlewaretoken'].value;
        ...
        datos_operation['insumo'] = document.getElementById('insumo').value;
