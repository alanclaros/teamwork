...
def myFirstPage(canvas, doc):
    ...
    datosReporte['titulo'] = 'Cobros Manuales ' + DATO_CAJA
...
def rptCobrosManuales(buffer_pdf, usuario, bloque_id, piso_id, caja_id, fecha_ini, fecha_fin, anulados, cobro_manual_id):
    # pdf
    ...
    datos_reporte = reporte_controller.datos_cobros_manuales(usuario, bloque_id, piso_id, caja_id, fecha_ini, fecha_fin, anulados, cobro_manual_id)
    # print(datos_reporte)
    ...
    for dato in datos_reporte:
        ...
        if caja_id != dato['caja_id']:
            ...
            data = []
            data.append(['Fecha', 'Periodo', 'C Manual', 'Bloque', 'Piso', 'Dpto', 'Monto'])
            filas = 0
            total = 0
        ...
        cobro_manual = Paragraph(dato['cobro_manual'])
        datos_tabla = [dato['fecha_cobro'], dato['periodo'], cobro_manual, bloque, piso, departamento, str(dato['monto_bs'])]
        data.append(datos_tabla)
        filas += 1
        total += dato['monto_bs']
