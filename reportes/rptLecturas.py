...
def myFirstPage(canvas, doc):
    ...
    datosReporte['titulo'] = 'Lecturas ' + DATO_CAJA
...
def rptLecturas(buffer_pdf, usuario, bloque_id, piso_id, caja_id, periodo_ini, periodo_fin, anulados):
    # pdf
    ...
    datos_reporte = reporte_controller.datos_lecturas(usuario, bloque_id, piso_id, caja_id, periodo_ini, periodo_fin, anulados)
    # print(datos_reporte)
    ...
    for dato in datos_reporte:
        ...
        if caja_id != dato['caja_id']:
            ...
            data = []
            data.append(['Fecha', 'Periodo', 'Bloque', 'Piso', 'Dpto', 'Lectura', 'Consumo'])
            filas = 0
            total = 0
        ...
        datos_tabla = [dato['fecha_cobro'], dato['periodo'], bloque, piso, departamento, str(dato['lectura']), str(dato['consumo'])]
        data.append(datos_tabla)
        filas += 1
        total += dato['consumo']
