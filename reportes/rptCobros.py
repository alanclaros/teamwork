...
def myFirstPage(canvas, doc):
    ...
    datosReporte['titulo'] = 'Cobros Realizados ' + DATO_CAJA
...
def rptCobros(buffer_pdf, usuario, bloque_id, piso_id, caja_id, fecha_ini, fecha_fin, anulados):
    # pdf
    ...
    datos_reporte = reporte_controller.datos_cobros(usuario, bloque_id, piso_id, caja_id, fecha_ini, fecha_fin, anulados)
    # print(datos_reporte)
    ...
    for dato in datos_reporte:
        ...
        if caja_id != dato['caja_id']:
            ...
            if bande > 1:
                ...
                if len(data) > 0:
                    ...
                    tabla_datos = Table(data, colWidths=[30*mm, 20*mm, 25*mm, 30*mm, 40*mm, 25*mm], repeatRows=1)
            ...
            data = []
            data.append(['Fecha', 'Periodo', 'Bloque', 'Piso', 'Dpto', 'Monto'])
            filas = 0
            total = 0
        ...
        datos_tabla = [dato['fecha_cobro'], dato['periodo'], bloque, piso, departamento, str(dato['monto_bs'])]
        data.append(datos_tabla)
        filas += 1
        total += dato['monto_bs']
